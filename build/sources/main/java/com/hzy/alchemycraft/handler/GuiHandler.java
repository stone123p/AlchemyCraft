...
public class GuiHandler implements IGuiHandler{
			...
			switch(ID){
				case AlchemyCraft.Distiller_GUIID:{
					if(entity instanceof TileEntityDistillerBlock ){
						return new ContainerDistiller(player.inventory,(TileEntityDistillerBlock ) entity);
					}
					break;
				}	
				case AlchemyCraft.AlchemyTable_GUIID:{
					if(entity instanceof TileEntityAlchemyTable ){
						return new ContainerAlchemyTable(player.inventory,(TileEntityAlchemyTable) entity);
					}
					break;
				}	
			}
	...
	public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		...
		if(entity != null){
			...
			case AlchemyCraft.Distiller_GUIID:{
				if(entity instanceof TileEntityDistillerBlock ){
					return new GuiDistiller(player.inventory,(TileEntityDistillerBlock ) entity);
				}
				break;
			 }
			case AlchemyCraft.AlchemyTable_GUIID:{
				if(entity instanceof TileEntityAlchemyTable ){
					return new GuiAlchemyTable(player.inventory,(TileEntityAlchemyTable ) entity);
				}
				break;
			}	
			}
