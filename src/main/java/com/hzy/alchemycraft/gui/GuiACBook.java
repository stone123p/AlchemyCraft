...
public class GuiACBook extends GuiScreen{
	...
	protected void mouseClicked(int x, int y, int button) {
		...
		if(guiX+135<=x&&x<=guiX+163&&guiY+50<=y&&y<=guiY+72){
			MaxPageNumber=DistillerRecipes.RecipesList.length/4+2;
			pageNumber=0;
			Mainpage=1;
		}
	...
	public void SelectDistill(int x,int y){
		...
		if(pageNumber>=3){
			...
			DrawPotionInformation(DistillerRecipes.RecipesList[i*2+3].getItem(),i+1,x+20,y+105);
		}
	...
	public void DrawPotionInformation(Item potion,int potionIndex,int x,int y){
		...
		String data=
				...
				"\n配方："+Materials+
				"\n效果："+StatCollector.translateToLocal(ItemPotion.getEffects(new ItemStack(potion)).getEffectName())+"Level:"+RomanNumerals[ItemPotion.getEffects(new ItemStack(potion)).getAmplifier()];
