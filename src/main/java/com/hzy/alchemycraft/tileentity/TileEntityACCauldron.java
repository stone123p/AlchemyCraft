...
public class TileEntityACCauldron extends TileEntity implements  ITickable{
	...
	public int dePotion(ArrayList<ItemStack> products,ArrayList<ItemStack> potionsm ,int[] RSize){
		...
		for(int i=0;i<RSize[1];i++){
			for(ItemStack P:potionsm){
				if(P.getItem()!=Items.potionitem && P.getItem()==products.get(RSize[0]+i).getItem()){
			...
			for(int i=0;i<entityItem.size();i++){
				if(products.get(index[0]+j)!=null)
					if(entityItem.get(i).getEntityItem().getItem()==ACItems.HigherPhilosopherStone){
						...
						entityItem.get(i).getEntityItem().setItemDamage(entityItem.get(i).getEntityItem().getItemDamage()+25);
						if(entityItem.get(i).getEntityItem().getItemDamage()>=entityItem.get(i).getEntityItem().getMaxDamage())entityItem.get(i).setEntityItemStack(null);
					}else 
	...
	public void addPotion(ItemStack potion) {
		...
		for(int i=0;i<potions.size();i++){
			if(potions.get(i).getItem()==potion.getItem()){
				potions.get(i).stackSize+=addPotionMole(potion);
				
				j++;
			}
		}
		if(j==0){
			potions.add(new ItemStack(potion.getItem(),addPotionMole(potion)));
		}		
	}
	...
	public int addPotionMole(ItemStack poion){
		if(poion.getMetadata()!=0)return poion.getMetadata();
		return StandardMole(poion.getItem());
	}
}
