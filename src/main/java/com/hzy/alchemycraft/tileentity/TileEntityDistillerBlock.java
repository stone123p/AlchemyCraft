...
public class TileEntityDistillerBlock  extends TileEntity implements IInventory, ITickable{
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			if(this.furnaceItemStacks[0].getItem()!=ACItems.PotionDistilledBottle){
				...
				ItemStack j=furnaceItemStacks[0];
				ItemStack itemstack = DistillerRecipes.smelting().getSmeltingResult(j,0);
				...
				furnaceItemStacks[0]=NewBottol(furnaceItemStacks[0],itemstack);
				 
				if(((NBTTagCompound)furnaceItemStacks[0].getTagCompound().getTag("potion")).getInteger("size")==0){
					--this.furnaceItemStacks[0].stackSize;
					this.furnaceItemStacks[0]=new ItemStack(ACItems.DistilledBottle);
					if(this.furnaceItemStacks[0].stackSize == 0){
						this.furnaceItemStacks[0] = null;
	...
	public ItemStack  NewBottol(ItemStack org,ItemStack product){
		ItemStack PDB=new ItemStack(org.getItem(),1);
		...
		if(org.getTagCompound() !=null){
			if(org.getTagCompound().hasKey("potion")){
				...
				if(nbt.getInteger("n_"+String.valueOf(nbt.getInteger("size")-1))>StandardMole(product.getItem())){
					nbt.setInteger("n_"+String.valueOf(nbt.getInteger("size")-1),nbt.getInteger("n_"+String.valueOf(nbt.getInteger("size")-1))-StandardMole(product.getItem()));
				}else{
