...
public class Distiller  extends BlockContainer implements ITileEntityProvider {
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		if(!worldIn.isRemote){
			for(int i=0; i<tileEntity.furnaceItemStacks.length;i++){
				if(tileEntity.furnaceItemStacks[i]!=null)worldIn.spawnEntityInWorld(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), tileEntity.furnaceItemStacks[i]));
			}
		}
