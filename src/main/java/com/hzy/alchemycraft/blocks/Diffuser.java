...
public class Diffuser extends BlockContainer implements ITileEntityProvider {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		 ...
		 this.TileEntity = (TileEntityDiffuserBlock)worldIn.getTileEntity(pos);
	        ...
	        if (HeldItem != null) {
	            if (ItemPotion.getEffects(HeldItem) != null) {
	...
	public boolean onBlockEventReceived(World worldIn, BlockPos pos, IBlockState state, int eventID, int eventParam) {
		if (eventParam == 0) {
			for(int i=0;i<2;i++)worldIn.spawnParticle(EnumParticleTypes.SPELL_WITCH, (double)pos.getX() + 0.5, (double)pos.getY() + 0.8, (double)pos.getZ() + 0.5, 0.0, 0.0, 0.0);
        }
		return true;
	}
